...
public class KingSafetyEvaluator : IEvaluationComponent
{
    ...
    private static int EvaluateWhiteKing(ChessBoard board)
    {
        ...
        while (tempWhiteKing.Value != 0)
        {
            ...
            if ((board.WhitePawns & PieceSquareTables.FileMasks[cell]).Value == 0)
                score -= PieceSquareTables.SemiOpenFileScore;

            if ((board.WhitePawns & board.BlackPawns & PieceSquareTables.FileMasks[cell]).Value == 0)
                score -= PieceSquareTables.OpenFileScore;

            score += (KingMoveGenerator.KingAttackMasks[cell] & board.WhiteOccupancies).CountBits() *
                     PieceSquareTables.KingShieldBonus;
        }
    ...
    private static int EvaluateBlackKing(ChessBoard board)
    {
        ...
        while (tempBlackKing.Value != 0)
        {
            ...
            if ((board.BlackPawns & PieceSquareTables.FileMasks[cell]).Value == 0)
                score += PieceSquareTables.SemiOpenFileScore;

            if ((board.BlackPawns & board.WhitePawns & PieceSquareTables.FileMasks[cell]).Value == 0)
                score += PieceSquareTables.OpenFileScore;

            score -= (KingMoveGenerator.KingAttackMasks[cell] & board.BlackOccupancies).CountBits() *
                     PieceSquareTables.KingShieldBonus;
        }
